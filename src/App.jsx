...
const DICT = {
  zh: {
    ...
    nav_texts: "思想摘錄",
    nav_partners: "合作提案",
    hero_title: "這不是占卜。這是一個讓你看見「此刻唯一」的工具。",
    hero_init_desc: "這一副牌的排列，幾乎不可能在宇宙中重複出現。\n每一次洗牌，都是一次只屬於此刻的排列。\n52 張牌，可以排列出 52! 種可能。\n你現在看到的這一組，就是此刻唯一的時空書籤。",
    hero_manifested_desc: "這一刻，不會再重來。\n不是因為它特別神祕，而是因為它真的只發生一次。\n這是你的此刻。",
    hero_init: "這是你的此刻",
    btn_manifest: "產生我的此刻排列",
    quote_default: "不是你抽中了什麼，而是你看見了此刻。",
    ...
    portal_desc: "如果這個小工具讓你停下來一秒，也許它已經完成了它的目的。你也可以將此片刻轉化為實體存在。",
    portal_notice: "系統已為此刻建立 Artwork ID。請進入表單填寫尺寸、款式與收件資料。營運端將依據表單中的牌序重新產生工廠印刷圖檔。客製商品不適用七天鑑賞期退換貨。",
  ...
  en: {
    ...
    hero_title: "This is not fortune-telling. It is a tool to witness 'The Only Moment'.",
    hero_init_desc: "This exact arrangement of cards is nearly impossible to repeat in the universe.\nEvery shuffle is an arrangement that belongs only to this moment.\n52 cards create 52! possibilities.\nWhat you see now is the unique space-time bookmark of the present.",
    hero_manifested_desc: "This moment will never return.\nNot because it is mystical, but because it truly only happens once.\nThis is your moment.",
    hero_init: "This is your moment",
    ...
    portal_desc: "If this tool made you pause for a second, it has served its purpose. You can also turn this moment into physical existence.",
  ...
  const renderHome = () => (
    <motion.main initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex flex-col items-center justify-center px-5 pt-28 pb-32 min-h-screen">
      <section className="w-full max-w-[760px] text-center">
        ...
        {/* Core Positioning */}
        <div onClick={handleSecretClick} className="mb-8 text-[0.8rem] font-medium tracking-[0.2em] text-emerald-500/90 cursor-default select-none">
          {t.hero_title}
        </div>
        
        <p className="mb-12 text-[0.95rem] font-light leading-[2] text-neutral-400 whitespace-pre-line">
          {manifested ? t.hero_manifested_desc : t.hero_init_desc}
        </p>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={signature + String(manifested)} initial={{ opacity: 0, filter: "blur(8px)" }} animate={{ opacity: fading ? 0.28 : 1, filter: fading ? "blur(6px)" : "blur(0px)" }} transition={{ duration: 0.5 }}
            className="mb-8 min-h-[146px] break-words rounded-[4px] border border-white/[0.05] bg-white/[0.015] px-6 py-8 text-[1.1rem] font-light leading-[2] tracking-[0.08em] text-white shadow-2xl"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-16 border-t border-white/[0.05] text-left">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => setShowPortal(true)} className="inline-flex items-center justify-center gap-2 border border-emerald-500/20 bg-emerald-500/5 px-4 py-2.5 text-xs text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/10 transition"><Shirt className="h-3.5 w-3.5" /> {t.btn_order}</button>
            </div>
          </div>
        </div>
