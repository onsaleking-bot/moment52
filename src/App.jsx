...
export default function App() {
  ...
  const handleDownload = async () => {
    ...
    try {
      await exportElementAsPng(cardRef.current, `52Moment-${momentData.signature}.png`);
    } catch (error) {
  ...
  const handleFactoryExport = async () => {
    ...
    try {
      await exportElementAsPng(factoryRef.current, `FACTORY-PRINT-${momentData.signature}.png`, {
        scale: 4,
  ...
  const renderHomeView = () => (
    ...
    >
      ...
      <p className="mb-16 text-xl font-light tracking-widest text-neutral-400 md:text-2xl">
        此刻唯一。
      </p>
    ...
    >
      <div
        className={`text-center transition-all duration-1000 ${
          showInput ? "mb-12 opacity-30" : "mb-0 opacity-100"
        }`}
      >
      ...
      <div className="grid w-full items-center gap-16 md:grid-cols-2">
        <div className="relative mx-auto flex aspect-[3/4] w-full max-w-sm items-center justify-center overflow-hidden border border-white/10 bg-[#111]">
          ...
          <div className="relative z-10 mt-4 flex w-full flex-col items-center p-8 text-center">
            ...
            <p className="mb-16 font-mono text-[8px] tracking-[0.2em] text-neutral-600 md:text-[10px]">
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#0E0E0E] font-sans text-[#D4D4D4] selection:bg-white/30">
      {momentData && (
